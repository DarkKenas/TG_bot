...
from middlewares import DIMiddleware, RegistrationMiddleware, RequireAdmin, RequireServiceUser
...
async def main():
    ...
    scheduler.add_job(
        send_birthday_notifications, "cron", hour=9, minute=0, args=(bot, 7, pg_db)
    )
    scheduler.add_job(
        send_birthday_notifications, "cron", hour=9, minute=0, args=(bot, 1, pg_db)
    )
